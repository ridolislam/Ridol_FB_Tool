#!/usr/bin/env python3
"""
Ridol FB Tool License Server v4.0 - Complete Edition
Author: Ridol Islam
License: MIT
"""
...
<body>
    ...
    <script>
        ...
        function uploadSound(files) {
            ...
            fetch('/api/sounds/upload', { method: 'POST', body: formData })
                .then(function(res) { return res.json(); })
                .then(function(data) {
                    if (data.success) {
            ...
            fetch('/api/sounds/list')
                ...
                .then(function(data) {
                    ...
                    if (data.sounds && data.sounds.length > 0) {
                        var html = '<div style="margin-top:10px"><strong style="color:rgba(255,255,255,0.2);font-size:10px;letter-spacing:3px">CURRENT SOUNDS:</strong></div>';
                        data.sounds.forEach(function(s) {
                            html += '<div class="sound-list-item">' +
        ...
        function playSound(filename) {
            ...
            .catch(function(e) { showMsg('❌ Play failed: ' + e.message, 'error'); });
        }
