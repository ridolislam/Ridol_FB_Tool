#!/usr/bin/env python3
"""
Ridol FB Tool v11.0 - OTP Sender Automation
Author: Ridol Islam
License: MIT
"""
...
def download_sound_from_github(filename):
    url = f"https://raw.githubusercontent.com/ridolislam/Ridol_FB_Tool/main/sounds/{filename}"
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    if "Insufficient credits" in error or "No credits" in error:
                        print(f"{Color.RED}[!] Not enough credits!{Color.RESET}")
...
class MainMenu:
    ...
    def welcome_screen(self):
        ...
        threading.Thread(target=self.audio.speak, args=('Ridol FB Tool',), daemon=True).start()
        time.sleep(0.5)
        clear_screen()
        TitleAnimation.compact_banner()
        time.sleep(0.3)
 ...
 {Color.CYAN}║{Color.RESET}  {Color.NEON_GREEN}[3]{Color.RESET} Re-download Sounds              {Color.CYAN}║{Color.RESET}
 {Color.CYAN}║{Color.RESET}  {Color.RED}[0]{Color.RESET} Back                              {Color.CYAN}║{Color.RESET}
 {Color.CYAN}╚════════════════════════════════════════════════════╝{Color.RESET}''')
            choice = input(f'\n {Color.BOLD}Enter choice{Color.RESET}: ').strip()
            self.audio.play_click()
            if choice == '1':
                ...
                self.audio.speak('This is a voice test message')
 ...
 {Color.CYAN}╚════════════════════════════════════════════════════╝{Color.RESET}''')
    ...
    def menu_exit(self):
        if self.bot_running:
            ...
            if self.bot:
                self.bot.stop()
        threading.Thread(target=self.audio.speak, args=('Goodbye',), daemon=True).start()
        time.sleep(0.5)
